...
# ✅ Ensure your URI ends with a DB name (e.g., calmapp)
...
def signup():
    users = mongo.db.users  # ✅ moved inside
    data = request.json
...
def prompt():
    prompts = mongo.db.prompts  # ✅ moved inside
    data = request.json
...
def prompt_history():
    ...
    results = list(prompts.find({'email': str(email)}, {'_id': 0}))
